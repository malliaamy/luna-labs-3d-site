...
vec3 lunaCreaturePaint = mix(
  vec3(0.10, 0.015, 0.25),
  vec3(0.62, 0.16, 0.96),
  clamp(lunaSurface * 0.9 + 0.12, 0.0, 1.0)
);
...
vec4 diffuseColor = vec4(lunaPaint, opacity);`)
    }, e.customProgramCacheKey = () => `luna-purple-dragon-v10`, e
}
...
function w() {
    ...
    return (0, b.useEffect)(() => {
        ...
        e.resin && n.current.forEach(t => {
            let n = t;
            for (; n && !/^(Head|Tail|Body|Wing1|Wing2)$/.test(n.name);) n = n.parent;
            t.material = O === `painted` ? n ? e.painted : e.resin : e[O]
        })
    }, [O]), (0, x.jsxs)(`div`, {
