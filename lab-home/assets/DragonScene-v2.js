...
import {
    ...
    u as y,
    w as q
} from "./meshopt_decoder.module-CoX9bj3I.js";
var b = e(t(), 1),
    x = n(),
    S = [{
        id: `sculpt`,
...
vec3 lunaMoss = vec3(0.37, 0.58, 0.035) * (0.75 + lunaSurface * 0.45);

vec3 lunaPaint = mix(lunaStone, lunaCreaturePaint, lunaCreature);
lunaPaint = mix(lunaPaint, lunaMoss, lunaMossMask * 0.72);

float lunaBrush = 0.93 + 0.09 * sin(
  lunaP.y * 94.0 + lunaSurface * 8.0 + lunaP.x * 13.0
);
lunaPaint *= lunaBrush;
lunaPaint += vec3(0.16, 0.03, 0.22)
  * smoothstep(0.74, 0.95, lunaDetail)
  * (0.25 + lunaCreature * 0.75);

vec4 diffuseColor = vec4(lunaPaint, opacity);`)
    }, e.customProgramCacheKey = () => `luna-procedural-paint-v2`, e
}
...
function w() {
    ...
    return (0, b.useEffect)(() => {
        ...
        let k = new _;
        k.rotation.set(-.04, Q(), 0), x.add(k), t.current = k, window.__lunaGroup = k, window.__lunaD = D;
        let A = {
            sculpt: new q({
                color: 13172541,
                wireframe: !0,
